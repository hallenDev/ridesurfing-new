...
class TripCard extends Component {
  constructor(props) {
    super(props);
    this.state = {
      render_status: this.props.render_status || false,
      render_menu: this.props.render_menu || false,
  ...
  renderStatus(trip_status) {
    ...
    if (
      !this.props.render_status ||
      typeof trip_status === "undefined" ||
      !(trip_status in options)
    )
      return "";

    const stat = this.state.trip_cancel_sent
      ? options.cancelled
      : options[trip_status];
    return (
  ...
  renderMenu() {
    if (!this.state.render_menu) return "";

    return (
      <TripMenu
        trip={this.props.trip}
        on_menu_open={() => {
          this.setState({ on_click_active: false });
        }}
        on_menu_closed={() => {
          this.setState({ on_click_active: true });
        }}
        on_trip_cancelled={(id) => {
          this.setState({ trip_cancel_sent: true, trip_rendered: false });
        }}
        on_request_cancelled={(id) => {
          this.setState({ request_cancel_sent: true, trip_rendered: false });
        }}
      />
    );
  }
  ...
  render() {
    ...
    if (this.props.onMouseEnter) {
      mouse_enter_cb = () => this.props.onMouseEnter(trip);
    }
    if (this.props.onMouseLeave) {
      mouse_leave_cb = () => this.props.onMouseLeave();
    }
            ...
            <div className="main">
              ...
              <div className="right-box">
                ...
                <div className="mr10">
                  ...
                  {this.renderStatus(trip.attributes.status)}
