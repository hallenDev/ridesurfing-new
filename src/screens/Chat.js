...
const Chat = ({cable, ...props}) => {
  ...
  }, [cable.subscriptions, chatUser.id, currentUser.id])
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [cable.subscriptions, subscribeChannel])
  ...
  const userTile = (user, currentChat) => {
    return (
      <li
        className={'user ' + currentChat}
        onClick={() => loadChat(user.id)}
        key={'userTile' + user.id}>
        <div
          className={`user-link clearfix ${user.attributes.unread > 0 &&
            'unread-msg-block'}`}>
          <div className="img-container">
            <img
              src={user.attributes.small_image_url || missingImg}
          ...
          <small
            className={`time ${user.attributes.unread > 0 && 'unread-color'}`}>
        ...
        <div className="row mb0">
          <div className="row chat-list-sm">
            ...
            <div className="col s3">
              <Link
                to={`/profile/${chatUser.attributes.slug || chatUser.id}`}
                className="user-img-container">
                <img
                  src={chatUser.attributes.small_image_url || missingImg}
            ...
            <div className="row mb0">
              ...
              <div className="col l8 s12 pl0 pr0">
                <div className="chat-block">
                  ...
                  <Chats messages={chats} onSend={sendChat} />
                </div>
              </div>
            </div>
          </div>
